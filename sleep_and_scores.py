...
def compute_regression(sleep,scores):
    ...
    #slope = np.sum(normalized_sleep * normalized_scores) / sum(normalized_sleep * normalized_sleep)
    slope = sum(normalized_sleep*normalized_scores) /  sum(normalized_sleep * normalized_sleep)
    #(np.sum(scores * normalized_sleep)) / np.sum(np.square(normalized_sleep))
    print (slope)

    #	Finally, We have a linear function of the form
    #	y - avg_y = slope * ( x - avg_x )
    #   y = slope * ( x - avg_x ) + avg_score
